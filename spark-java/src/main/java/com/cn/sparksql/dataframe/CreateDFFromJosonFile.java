...
public class CreateDFFromJosonFile {
	...
	public static void main(String[] args) {
		 ...
		 * 不能读取嵌套格式的json数据
		 */
		Dataset<Row> df = sqlContext.read().format("json").load("./sparksql/json");
...
//		df.printSchema();
		...
		Dataset<Row> sql = sqlContext.sql("select age,count(*) as gg from jtable group by age");
		sql.show();
		Dataset<Row> sql1 = sqlContext.sql("select name,age from jtable");
		sql1.show();
		sc.stop();
	}
}
